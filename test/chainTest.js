...
function encrptPwd(pwd){
	var	password = util.encode(pwd, config.decode);
	console.log(pwd," encode: ", password);
	var password = util.decode(password, config.decode);
	console.log(pwd," decode: ", password);
}
encrptPwd("$z58dSHE");
