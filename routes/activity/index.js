...
exports.post = function*() {
	...
	if (this.request.query.id) {
		...
		console.log('czz修改活动:',"args "+JSON.stringify(args))
		
		try {
			var result = yield chain.invoke('oneLotteryEdit', ['', args]);
			console.log('czz修改活动result:',result)
			...
			if (result.code == 0) {
				...
				this.response.body = 'czz修改活动成功！';
			} else {
				this.response.body = 'czz修改活动失败！'
			}
		} catch (e) {
			console.log(e);
			this.response.body = 'czz修改活动失败！'
	...
	args = JSON.stringify(args);
	console.log('czz创建活动args:',args);
	try {
		var result = yield chain.invoke('oneLotteryAdd', ['', args]);
		console.log('czz创建活动result:',result)
